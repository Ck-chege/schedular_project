import { getShiftCycleWithWorkdaysAndShifts } from "@/actions/shiftCyclesActions";
import EmployeeShiftAvailabilityPage from "./EmployeeShiftAvailabilityPage";



export default async function ConfigureScheduleCyclePage({ params }: { params: { cycle_id: string } }) {
  const { data:shiftCycle, error } = await getShiftCycleWithWorkdaysAndShifts(params.cycle_id);

  if (error) {
    console.error("Error getting shift cycle:", error);
    return <div>Error fetching shift cycle</div>;
  }
  return (
    <div className="w-full p-6">
        {shiftCycle && <EmployeeShiftAvailabilityPage shiftCycle={shiftCycle}/>}
      </div>
  )
}
